...
### Phase B（已完成 ✅，observability-first 简化版）—— Engine 接线

落地内容（保守路径，不触碰 execution 时机）：
1. **`engine.rs` forwarder** 识别 `ToolUseStart` chunk → emit `tool_use_started` agent-stream 事件（meta 含 tool_name + tool_use_id），让前端在 LLM 还没收完时已显示 spinner
2. **`ToolUseStop` chunk** → emit `tool_use_arg_done` 事件让前端把"args 解析中"切换到"等执行"
3. `ToolUseInputDelta` / `MessageStop` 不透传——前端不需要看 JSON 半成品

**未做**（留作未来 PR）：真正的 streaming overlap（safe tool 在 stream 中途启动）。涉及 ToolDispatcher trait 接入 + cancel/approval 边界 + drain_in_order 替换 batch 路径，估计 300 行 + 多个集成测试。当前 observability shim 已经给用户**"看起来快了 1-3s"**的感知 win，且 100% 零风险（execution 路径不变，与原 batch 行为字节级等价）。

测试结果：541 → 562 全过（无新增测试，纯协议透传）。
