...
struct AggregateData {
    ...
    delivery: Option<crate::agent::delivery::MissionDeliverySnapshot>,
...
fn aggregate_data(conn: &Connection, mission_id: &str) -> Result<AggregateData> {
    ...
    let delivery = queries::get_mission_delivery(conn, mission_id)
        ...
        .flatten()
        .and_then(|row| {
            serde_json::from_str::<crate::agent::delivery::MissionDeliverySnapshot>(
                &row.snapshot_json,
            )
            .ok()
        });
...
pub fn render_markdown(report: &MissionReport) -> String {
    ...
    if let Some(delivery) = &report.delivery {
        ...
        md.push_str(&format!("{}\n\n", escape_md(&delivery.overview.summary)));
        ...
        if !delivery.how_to_use.is_empty() {
            md.push_str("### How to use\n\n");
            for step in &delivery.how_to_use {
                md.push_str(&format!(
                    "- **{}**: {}\n",
                    escape_md(&step.title),
                    escape_md(&step.detail)
                ));
            }
            md.push('\n');
        }
...
mod tests {
    ...
    fn markdown_includes_delivery_snapshot_section() {
        let mut report = make_test_report();
        report.delivery = Some(crate::agent::delivery::MissionDeliverySnapshot {
            schema_version: crate::agent::delivery::DELIVERY_SNAPSHOT_SCHEMA_VERSION,
            mission_id: "m1".into(),
            status: crate::agent::delivery::DeliveryStatus::CompletedWithWarnings,
            confidence: crate::agent::delivery::DeliveryConfidence::High,
            overview: crate::agent::delivery::DeliveryOverview {
                title: "Build app".into(),
                summary: "Delivered app bundle.".into(),
                status: crate::agent::delivery::DeliveryStatus::CompletedWithWarnings,
                confidence: crate::agent::delivery::DeliveryConfidence::High,
            },
            items: vec![crate::agent::delivery::DeliveryItem {
                id: "app".into(),
                source: crate::agent::delivery::DeliveryCandidateSource::Artifact,
                title: "App.app".into(),
                summary: "Runnable app bundle.".into(),
                file_paths: vec!["dist/App.app".into()],
                confidence: crate::agent::delivery::DeliveryConfidence::High,
            }],
            how_to_use: vec![crate::agent::delivery::HowToUseStep {
                title: "Open".into(),
                detail: "Open dist/App.app.".into(),
            }],
            validation: vec![],
            changes: vec![],
        ...
        assert!(markdown.contains("Not notarized"));
    }
