...
impl Scheduler {
    ...
    fn poll_and_dispatch(
        ...
        if slots <= 0 {
            ...
            return Self::check_and_handle_terminal(mission_id, repo_path, app);
        }
        ...
        Self::check_and_handle_terminal(mission_id, repo_path, app)
    }

    fn check_and_handle_terminal(
        mission_id: &str,
        repo_path: &PathBuf,
        app: &tauri::AppHandle,
    ) -> Result<bool> {
        ...
        if let Some(new_status) = terminal.as_deref() {
            let action = Self::run_terminal_delivery_side_effects(&db, mission_id, new_status);
            if action.spawn_merge {
                // FM-15 FR-08.2 (3): merge 路径含 async LLM 解冲突，必须在 tokio task 里跑。
                // 这里 spawn 不阻塞 poll loop 的退出；delivery snapshot 已在 spawn 前 best-effort 落库。
                let mid = mission_id.to_string();
    ...
    fn run_terminal_delivery_side_effects(
        db: &Database,
        mission_id: &str,
        new_status: &str,
    ) -> TerminalMissionAction {
        if matches!(new_status, "completed" | "failed") {
            Self::generate_terminal_delivery_best_effort(db, mission_id);
        }
        TerminalMissionAction {
            spawn_merge: new_status == "completed",
        }
    }

    fn generate_terminal_delivery_best_effort(db: &Database, mission_id: &str) {
    ...
    async fn merge_completed_mission(
        ...
        // The terminal degraded snapshot is generated by scheduler terminal handling before
        // merge is spawned, so consumers can fetch delivery synchronously when this event fires.
    ...
    fn build_agent_run_options(
        ...
        Ok(AgentRunOptions {
            ...
            // Single-Agent Uplift P0-2 (Phase B): 从 AppConfig plumb 预算。
            // 0 视为 None 关闭，保持向后兼容；非 0 启用 BudgetTracker。
            output_token_budget: if cfg.agent_output_token_budget == 0 {
                None
            } else {
                Some(cfg.agent_output_token_budget)
            },
            // Single-Agent Uplift P1-2 (Phase B): 从 AppConfig plumb fallback 模型。
            // 空字符串视为 None 关闭。**不做**等同主模型校验——用户故意把 fallback
            // 设成主模型本身在错误注入测试场景有用（强制重试当前模型）。engine 里有
...
mod tests {
    use super::*;
    use crate::agent::delivery::{DeliveryStatus, MissionDeliverySnapshot};
    use rusqlite::params;

    fn insert_terminal_mission(db: &Database, mission_id: &str, status: &str) {
        db.with_conn(|conn| {
            conn.execute(
                "INSERT INTO missions (id, title, description, status) VALUES (?1, 'Terminal Mission', 'Build delivery', ?2)",
                params![mission_id, status],
            )?;
            conn.execute(
                "INSERT INTO tasks (id, mission_id, title, description, status) VALUES (?1, ?2, 'Terminal Task', 'Produce output', ?3)",
                params![format!("{mission_id}-task"), mission_id, status],
            )?;
            Ok(())
        })
        .expect("seed terminal mission");
    }

    #[test]
    fn terminal_delivery_side_effect_persists_failed_snapshot_before_status_emit() {
        let db = Database::open_in_memory().expect("open in-memory db");
        insert_terminal_mission(&db, "mission-failed", "failed");

        let action = Scheduler::run_terminal_delivery_side_effects(&db, "mission-failed", "failed");

        assert!(!action.spawn_merge);
        let row = db
            .with_conn(|conn| queries::get_mission_delivery(conn, "mission-failed"))
            .expect("read mission delivery")
            .expect("failed terminal delivery is persisted synchronously");
        assert_eq!(row.generation_status, "degraded");
        let snapshot: MissionDeliverySnapshot =
            serde_json::from_str(&row.snapshot_json).expect("snapshot parses");
        assert_eq!(snapshot.status, DeliveryStatus::Failed);
    }

    #[test]
    fn terminal_delivery_side_effect_persists_completed_snapshot_and_requests_merge() {
        let db = Database::open_in_memory().expect("open in-memory db");
        insert_terminal_mission(&db, "mission-completed", "completed");

        let action =
            Scheduler::run_terminal_delivery_side_effects(&db, "mission-completed", "completed");

        assert!(action.spawn_merge);
        let row = db
            .with_conn(|conn| queries::get_mission_delivery(conn, "mission-completed"))
            .expect("read mission delivery")
            .expect("completed terminal delivery is persisted before merge spawn");
        assert_eq!(row.generation_status, "degraded");
        let snapshot: MissionDeliverySnapshot =
            serde_json::from_str(&row.snapshot_json).expect("snapshot parses");
